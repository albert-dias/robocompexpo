        ...
        <div class="card m-b-30">
            <div class="card-body">
                ...
                <div class="form-group m-b-0">
                    ...
                    <?= $this->Html->link('Cancelar', ['action' => 'gerador'], ['escape' => false, 'class' => 'btn btn-secondary waves-effect m-l-5']) ?>
                </div>
                <?= $this->Form->end() ?>
            </div>
        </div>
...
<script>
        ...
        $("#cpf").keyup(function () {
            ...
            var tamanho = $("#cpf").val().length;
                ...
                if(tamanho === 14){
                    ...
                    var cpf= $(this).val();
                    cpfcnpj = cpf;
                    console.log(validaCpfCnpj(cpfcnpj))
    ...
    async function getEnderecoCorreios1(el) {
        if (el.value.replace('-', '').length >= 8) {
            $('#loading').show();
            await $.getJSON(window.location.protocol + "//viacep.com.br/ws/"+el.value.replace('-', '')+"/json/unicode/", function (data) {
                $('#loading').hide();
                if (data.bairro) {
