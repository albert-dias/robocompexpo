...
$general = [
    'label'       => 'Prestadores',
    'label_s'     => 'Criar novo prestador',
    'actual'      => 'Listar prestadores',
    'breadcrumbs' => []
];
...
$sort      = $this->request->params['paging']['Providers']['sort'];
$direction = $this->request->params['paging']['Providers']['direction'];
?>
    ...
    <div class="col-12">
        <div class="card m-b-30">
            <div class="card-body">
                ...
                <h4 class="mt-0 header-title">Lista de prestadores</h4>
                <hr>
                        ...
                        <table id="tech-companies-1" class="table  table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th data-priority="3">Categoria</i></th>
                            ...
                            <tbody>
                                <?php foreach ($providers as $provider): ?>
                                    <tr>
                                        <td>
                                            <?php if ($provider->person->image && trim($provider->person->image) != ''): ?>
                                                <img src="<?= $provider->person->image ?>" class="img-responsive img-circle" width="60" height="60" style="border-radius: 0%">
                                            <?php else: ?>
                                                <img src="img/user.png" class="img-responsive img-circle" width="60" height="60" style="border-radius: 0%">
                                            <?php endif; ?>
                                        </td>
                                        <td class="td-center"><?= $provider->id ?></td>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <?= $this->Html->link(__('<i class="fas fa-edit"></i>'), ['action' => 'add-step-one', $provider->person->cpf], ['escape' => false, 'class' => "btn btn-primary btn-sm"]) ?>
                                                <?= $this->Html->link(__('<i class="fas fa-search-plus"></i>'), ['action' => 'view', $provider->id], ['escape' => false, 'class' => "btn btn-info btn-sm"]) ?>
                                                <?= $this->Html->link(__('<i class="fas fa-file-image"></i>'), ['action' => 'upload', $provider->id, 'profile'], ['escape' => false, 'class' => "btn btn-success btn-sm"]) ?>
                                                <?= $this->Html->link(__('<i class="fas fa-paperclip"></i>'), ['action' => 'upload', $provider->id, 'attachment'], ['escape' => false, 'class' => "btn btn-success btn-sm"]) ?>
                                                <?= $this->Html->link('<i class="far fa-trash-alt"></i>', '#', ['data-toggle' => 'modal', 'data-target' => '#ConfirmDelete', 'data-action' => Router::url(array('action' => 'delete', $provider->id)), 'escape' => false, 'class' => 'btn btn-danger btn-sm'], false) ?>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
