...
class UploadController extends AppController {
    ...
    public function __construct($option = null, $initialize = true, $error_messages = null) {
        
        $this->ses = $_SESSION['Upload'];
//        debug($this->ses);
//        die();
        ...
        $this->options = array(
            'script_url' => Router::url('/upload/'.$this->ses['dir'].'/', true),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/upload/'.$this->ses['dir'].'/',
            //'upload_url' => Router::url('/upload/'.$this->ses['dir'].'/', true),
            ...
            'download_via_php' => $this->ses['download'],
            ...
            'accept_file_types' => $this->generate_files_types($this->ses['types_file']),
            // The php.ini settings upload_max_filesize and post_max_size
            // take precedence over the following max_file_size setting:
            'max_file_size' => $this->ses['max_file_size'] ?: 0,
            'min_file_size' => 1,
            // The maximum number of files for the upload directory:
            'max_number_of_files' => $this->ses['max_files'] ?: false,
...
//        );
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                $t = TableRegistry::get($this->ses['table']);
                $o = $t->get($this->ses['id']);
                $campo = $this->ses['column'];
                $o->{$campo} = 'upload/'.$this->ses['dir'].'/'.$this->ses['id'].'/'.$file->name;
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            $upload = $this->Upload->patchEntity($upload, $this->request->data);
    ...
    private function generate_files_types($types) {
        if ($types) {
            $str     = '/.(';
            $str    .= implode('|', $types);
            $str    .= ')$/i';
